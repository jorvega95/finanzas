...
- **MSI-04 · Calendario de cuotas:** la cuota 1 se carga en el statement al que TDC-05 asigne la fecha de compra; la cuota n en el n-ésimo corte siguiente. `estimated_charge_date = max(purchase_date, period_start)` del ciclo de cada cuota: la cuota 1 siempre queda en `purchase_date` (porque `period_start ≤ purchase_date` por construcción de TDC-05); las cuotas 2..n quedan en el `period_start` de su ciclo (primer día del periodo de cobro). Si los statements futuros no existen aún (TDC-11), la fecha se calcula proyectando cortes con TDC-02 y se reconcilia cuando el statement abre.
...
- **MSI-10 · Alta retroactiva:** el usuario puede registrar una compra MSI realizada antes de empezar a usar el sistema. Campos requeridos: `description`, `amount`, `currency`, `credit_card_id`, `purchase_date`, `months`, `category_id`. El sistema: (1) crea la transacción-madre con `installment_plan_id` asignado (excluida de agregados, MSI-03); (2) calcula el calendario completo de cuotas con TDC-05/MSI-04; (3) las cuotas con `estimated_charge_date ≤ hoy` se marcan automáticamente como `paid` (retroactivo — se asume que ya se pagaron); (4) las cuotas con `estimated_charge_date > hoy` quedan `pending`. Ninguna cuota retroactiva se asigna a `statement_id` para no alterar deuda ni saldos históricos. Aplican MSI-02, MSI-03, MSI-09.
